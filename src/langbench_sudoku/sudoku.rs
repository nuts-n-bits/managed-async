...
fn partial_verify(board: &[[usize; 9]; 9], x: usize, y: usize) -> bool {
	let base_x = (x / 3) * 3;
	let base_y = (y / 3) * 3;
	for i in 0..9 {
...
fn solve(board: &mut [[usize; 9]; 9], x: usize, y: usize) -> bool {
	let z = (x * 9) + y + 1;
	if z == 82 {
		return true
	}
...
fn verify(board: &[[usize; 9]; 9]) -> bool {
	for i in 0..9 {
...
fn read_line(line: Vec<char>, board: &mut [[usize; 9]; 9]) {
	let ascii_zero = '0';
	let ascii_dot = '.';
	for i in 0..9 {
		for j in 0..9 {
