...
func solve_line(wg *sync.WaitGroup, line string) {
	defer wg.Done()
	var board [9][9]int
...
func process_parallel(path string) {
...
func main() {
	...
	process_parallel(os.Args[1])
	var t1 time.Time = time.Now()
	fmt.Printf("[info] time taken: %v ms\n", t1.Sub(t0).Milliseconds())
}
