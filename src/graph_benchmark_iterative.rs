...
struct Vertex {
	next: Option<Rc<Vertex>>,
	prev: Option<Rc<Vertex>>,
	colour: Option<isize>,
	neighbours: Map<Rc<Vertex>, bool>,
}
...
struct vertex_colour_colour  {
	v: Rc<Vertex>,
	i: isize,
	j: isize,
}

fn magic_f(k: isize, n: isize) -> f64 {   
    (n as f64).powf(1.0 - (1f64 / (k - 1) as f64)).ceil()
}

fn k_ge_log_n(k: isize, n: isize) -> bool {
	let x = 1i64;
	for i in 0..k {
		x *= 2;
	}
...
fn newVertex(colour: Option<isize>) -> Rc<Vertex> {
	Vertex { 
        prev: None, 
        next: None, 
        colour,
...
impl Graph {
    ...
    fn head(self) -> Option<Rc<Vertex>> {
        return self.dummy.next
    }
    
    fn shift(self, v: *Vertex) {
    ...
    fn social_credit(self, bad *Vertex) {
        bad.sudoku()
        this.size--
